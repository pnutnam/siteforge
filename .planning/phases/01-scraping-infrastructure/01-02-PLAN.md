...
- Validates output with GoogleMapsSchema before returning
  ...
  <verify>
    <automated>cat src/scraping/scrapers/google-maps.ts | grep -q "extends BaseScraper" && cat src/scraping/scrapers/google-maps.ts | grep -q "GoogleMapsSchema"</automated>
  </verify>
...
- Handles login wall gracefully (noted for v2 authentication)
  ...
  <verify>
    <automated>cat src/scraping/scrapers/instagram.ts | grep -q "extends BaseScraper" && cat src/scraping/scrapers/instagram.ts | grep -q "engagement" && cat src/scraping/scrapers/instagram.ts | grep -q "sort"</automated>
  </verify>
...
```
  ...
  <verify>
    <automated>cat src/scraping/scrapers/facebook.ts | grep -q "extends BaseScraper" && cat src/scraping/scrapers/facebook.ts | grep -q "FacebookSchema"</automated>
  </verify>
...
- Review dates captured per SCRAPE-05 requirement
  ...
  <verify>
    <automated>cat src/scraping/scrapers/yelp.ts | grep -q "extends BaseScraper" && cat src/scraping/scrapers/yelp.ts | grep -q "YelpSchema"</automated>
  </verify>
...
- Up to 160 reviews per the requirement
  ...
  <verify>
    <automated>cat src/scraping/scrapers/google-reviews.ts | grep -q "extends BaseScraper" && cat src/scraping/scrapers/google-reviews.ts | grep -q "GoogleReviewsSchema"</automated>
  </verify>
...
- Returns results per source for status tracking
  ...
  <verify>
    <automated>cat src/scraping/scrapers/parallel.ts | grep -q "pLimit" && cat src/scraping/scrapers/parallel.ts | grep -q "GLOBAL_CONCURRENCY" && cat src/scraping/scrapers/parallel.ts | grep -q "Promise.all"</automated>
  </verify>
